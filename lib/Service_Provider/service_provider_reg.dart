...
import 'package:finders_v1_1/Service_Provider/CIPCRegistration.dart';
...
class _PartnerRegistrationPageState extends State<PartnerRegistrationPage> {
  ...
  Future<void> registerPartner() async {
    if (_formKey.currentState!.validate()) {
      ...
      bool isRegNumValid = await _isRegistrationNumberValid(_registrationNumberController.text);
      if (!isRegNumValid) {
        showDialog(
          context: context,
            ...
            actions: [
              TextButton(
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const CIPCRegistrationPage()),
                  );
                },
      ...
      try {
        ...
        String serviceProviderId = uuid.v4();
